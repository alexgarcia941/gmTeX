...
async function renderLatexInComposedView(event) {
    ...
    for (let i = 0; i < matches.length; i++) {
        ...
        const file = new File([blob], `latexImage${i}_delete.png`, { type: "image/png" });
    ...
    setTimeout(async () => {
        ...
        let generatedImgTags = updatedBodyHTML.match(/(<img\b[^>]*\balt=["'])([^"']*)_delete([^"']*)(["'][^>]*>)/g);

        //delete generated inline images and break tags
        if (generatedImgTags) {
            updatedBodyHTML = updatedBodyHTML.replace(/<img\b[^>]*\balt=["'][^"']*_delete[^"']*["'][^>]*>\s*<br\s*\/?>/g, '');
        }
        ...
        if (generatedImgTags) {
            ...
            for (const [latexText, data] of imageMap) {
                if (generatedImgTags[i]) {
                    const cleanedImgTag = generatedImgTags[i].replace(/_delete/g, '');
                    console.log(cleanedImgTag);
                    updatedBodyHTML = updatedBodyHTML.replace(data.placeholder, cleanedImgTag);
                }
                i++;
            }
